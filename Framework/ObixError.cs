...
namespace ObixClientLibrary.Framework
{
    ...
    public class ObixError
    {
        ...
        public ObixError(Type @Type, XElement ErrContract)
        {
            ...
            string href = ErrContract.ObixHref();
            if (href != null)
            {
                this.AuxErrorMessage = href;
            }

            string displayName = ErrContract.ObixDisplayName();
            if (displayName != null)
            {
    ...
    public class ObixErrorStack
    {
        ...
        readonly List<ObixError> errorStack = new List<ObixError>();
        ...
        public ObixResult Push(ObixError error)
        {
            ...
            ObixErrorEventArgs args = new ObixErrorEventArgs(error);
            ErrorAdded?.Invoke(null, args);
        ...
        public ObixResult<TObject> PushWithObject<TObject>(Type @Type, Exception ex, TObject obj = default)
        {
            return new ObixResult<TObject>(Push(new ObixError(@Type, (int)ObixResult.kObixClientException, ex)), obj);
        }

        public ObixResult<TObject> PushWithObject<TObject>(Type @Type, ObixResult Result, TObject obj = default)
        {
            return new ObixResult<TObject>(Push(Type, Result), obj);
        }

        public ObixResult<TObject> PushWithObject<TObject>(Type @Type, ObixResult Result, int AuxErrorCode, string AuxMessage, TObject obj = default)
        {
            return new ObixResult<TObject>(Push(new ObixError(@Type, (int)Result, AuxErrorCode, AuxMessage)), obj);
        }

        public ObixResult<TObject> PushWithObject<TObject>(Type @Type, ObixResult Result, string AuxMessage, TObject obj = default)
        {
